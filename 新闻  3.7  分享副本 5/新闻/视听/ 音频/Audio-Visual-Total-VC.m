...
@interface Audio_Visual_Total_VC ()<UICollectionViewDelegate, UICollectionViewDataSource,moreMideleDelgate,moviedelegate>
...
//    self.tool = [Audio_Tools shareTools];
}
...
-(void)scrollViewAction
{
    ...
    self.scrollV = [[UIScrollView alloc] initWithFrame:CGRectMake(self.view.bounds.size.width/2-40*WIDTH, 0, 100*WIDTH, self.view.bounds.size.height)];
...
//    Audio_Detail_VC *detail = [[Audio_Detail_VC alloc] init];
    self.tool.detail.idTotal = middle;
//    NSLog(@"%@",_tool.detail.idTotal);
//    [self.tool.detail.tableV reloadData];
...
{
    MovieViewController *mm = [[MovieViewController alloc] init];
    mm.path = movie;
